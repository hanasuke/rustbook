...
pub fn do_sort<T: Ord>(x: &mut [T], up: bool) {
  if x.len() > 1 {
...
mod tests {
  ...
  impl Student {
    fn nes(first_name: &str, last_name: &str, age: u8) -> Self {
      Self {
  ...
  fn sort_students_by_age_ascending() {
    let taro = Studnet::new("taro", "yamada", 16)
    let hanako = Studnet::new("hanako", "yamada", 14)
    let kyoko = Studnet::new("kyoko", "ito", 15)
    let ryosuke = Studnet::new("ryosuke", "hayashi", 17)
  ...
  fn sort_students_by_name_ascending() {
    ...
    assert_eq!(sort_by(&mut x, &|a, b| a.last_name.cmp(&b.last_name)
                                .then_with(|| a.first_name.cmp(&b.firstname))), Ok(()));
